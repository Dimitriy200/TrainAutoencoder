...
def check_start_train_and_save_mlflow():
    ...
    new_model = tr.start_train_and_save_mlflow(path_Train_data = ch_bar_Norm,
                                                path_Valid_Data = ch_bar_Norm,
                                                path_Predict_Data = ch_bar_Anom,
...
def check_choise_result_barrier():
    ...
    barr_line, res_aLL = tr.choise_barrier_line(path_choise_Normal = os.path.join(path_imp_data, "Choise_barrier_Normal.csv"),
    ...
    tp, fn, fp, tn = tr.get_confusuon_matrix_for_class(res_aLL[:,1],
                                           res_aLL[:,2]) 

    scatter_subplots(res_aLL)

    Prec = keras.metrics.Precision()
    Prec.update_state(res_aLL[:, 1],
                      res_aLL[:, 2])

    Rec = keras.metrics.Recall()
    Rec.update_state(res_aLL[:, 1],
                     res_aLL[:, 2])
    
    logging.info(f"Prec = {Prec.result()}")
    logging.info(f"Rec = {Rec.result()}")

    # F1 = keras.metrics.F1Score()
    # F1.update_state(res_aLL[:, 1],
    #                 res_aLL[:, 2])
    
    # logging.info(f"F1Score = {F1.result()}")



    # plt.matshow([[tp, tn],[fp, fn]])
    # plt.show()
    ...
    # np.savetxt(os.path.join(base_dir, "data", "raw", "tests", "mse_met_anom.csv"), res_aLL, delimiter=',')
